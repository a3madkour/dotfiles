		...
		(save-excursion
          ...
          (when (re-search-forward "^\\* Current Season:" nil t)
			...
			(let ((accomplished (read-string "What did you accomplish this season? ")))
              ...
              ;; Add accomplishments before the next heading
              (if (re-search-forward "^\\*" nil t)
                  (progn (beginning-of-line) (forward-line -1))
				(goto-char (point-max)))
              (end-of-line)
              (insert "\n  Accomplished: " accomplished)))
          ;; Insert new season at top, after any #+TITLE/#+CATEGORY lines
          (goto-char (point-min))
          (while (looking-at "^[#]")
			(forward-line 1))
          (when (looking-at "^$") (forward-line 1))
          (insert (format "* Current Season: %s (%s — %s)\n"
                          name
                          (format-time-string "%B %Y")
                          (format-time-string "%B %Y"
                                              (time-add (current-time)
														(days-to-time (* weeks 7))))))
          (insert (format "  :PROPERTIES:\n  :START: %s\n  :END:   %s\n  :END:\n"
                          (format-time-string "[%Y-%m-%d %a]")
                          (format-time-string "[%Y-%m-%d %a]"
                                              (time-add (current-time)
...
the file is not in `org-roam-directory' or has no title."
  ...
  (defun a3madkour/create-weekly-summary ()
	"Create or open a weekly summary file in org/weekly_summary/ named by the current week."
	(interactive)
	(let* ((directory (a3madkour/org-file "weekly_summary/"))
           ;; Format: Year-Week (e.g., 2026-W01.org)
           (filename (format-time-string "%G-W%V.org"))
      ...
      ;; If it's a new file, insert a title header
      (when (= (buffer-size) 0)
		(insert (format "#+TITLE: Weekly Summary: %s\n\n" (format-time-string "%G-W%V")))
		(save-buffer))))
#+end_src
** Adapted from doom
#+begin_src emacs-lisp
...
(setq org-capture-templates
  ...
  (setq org-agenda-custom-commands
		'(("d" "Dashboard"
		   ;; Section 1: Today's schedule and habits
		   ((agenda ""
					((org-agenda-span 'day)
					 (org-super-agenda-groups
					  '((:name "⏰ Scheduled" :time-grid t :scheduled today :order 0)
						(:name "🔁 Habits" :habit t :order 1)
						(:name "⚠️ Overdue" :scheduled past :deadline past :order 2)))))
			
			;; Section 2: This week's commitments
			(tags-todo "thisweek"
					   ((org-agenda-overriding-header "📌 This Week")))

			;; Section 3: Available tasks by energy (active projects only)
