...
(defun a3madkour/show-project-status ()
  "Display active and resting projects with goals."
  (interactive)
  (let ((active '())
        (resting '()))
    (with-current-buffer (find-file-noselect "~/org/projects.org")
         ...
         (when (= 1 (org-current-level))
           (let ((title (org-get-heading t t t t))
             ...
             (cond
              ...
              ((member "resting" (org-get-tags))
               (push (cons title goal) resting))))))
       "+project"))
    (with-output-to-temp-buffer "*Project Status*"
      ...
      (dolist (p (nreverse active))
        (princ (format "  • %s\n    → %s\n" (car p) (cdr p))))
      (princ (format "\nResting (%d):\n" (length resting)))
      (dolist (p (nreverse resting))
        (princ (format "  • %s\n    → %s\n" (car p) (cdr p)))))))

(defun a3madkour/toggle-project-active ()
  "Toggle the current project heading between :active: and :resting:."
  (interactive)
  ...
  (let ((tags (org-get-tags)))
    (cond
     ((member "active" tags)
      ...
      (org-toggle-tag "resting" 'on)
      (message "Project set to resting"))
     ((member "resting" tags)
      (let ((count (a3madkour/check-wip-limit)))
        (org-toggle-tag "resting" 'off)
        (org-toggle-tag "active" 'on)
        (message "Project activated (%d/%d active)"
                 (1+ count) a3madkour/project-wip-limit)))
     (t
      (let ((count (a3madkour/check-wip-limit)))
        (org-toggle-tag "active" 'on)
        (message "Project set to active (%d/%d)"
                 (1+ count) a3madkour/project-wip-limit))))))
...
When called interactively, also switches to the projects buffer."
  ...
  (let* ((name (read-string "Project name: "))
         ...
         (area (completing-read "Life area: "
                                ...
                                nil t))
         (status (completing-read "Status: " '("active" "resting") nil t))
...
"
 ...
 "pt"
 '(a3madkour/toggle-project-active :which-key "toggle project active")
 "pc"
