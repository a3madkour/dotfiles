...
#+begin_src emacs-lisp
  ...
  (use-package org
	...
	(defun a3madkour/add-new-org-datetree-headline (selected_date)
	  (save-excursion
		(setq parsed-string (nthcdr 3 (org-parse-time-string selected-date)))
		(setq new-date (list (nth 1 parsed-string) (car parsed-string) (nth 2 parsed-string)))
		(org-datetree-find-date-create new-date)
		(setq new-headline (nth 4 (org-heading-components)))))

	(defun a3madkour/refile-to (file headline selected-date)
	  "Move current headline to specified location"
	  (let ((pos (save-excursion
				   (find-file file)
				   (setq new_pos (org-find-exact-headline-in-buffer headline))
	...
	(defun a3madkour/refile-and-schedule ()
	  (interactive)
	  (setq selected-date (org-read-date))
...
#+begin_src emacs-lisp
  (general-define-key
   ...
   ;;org-roam
   "m" '(nil :which-key "org-roam")
   "mD" #'org-roam-demote-entire-buffer
   "mf" #'org-roam-node-find
   "mF" #'org-roam-ref-find
   "mg" #'org-roam-graph
   "mi" #'org-roam-node-insert
   "mI" #'org-id-get-create
   "mm" #'org-roam-buffer-toggle
   "mM" #'org-roam-buffer-display-dedicated
   "mn" #'org-roam-capture
   "mr" #'org-roam-refile
   "mR" #'org-roam-link-replace-all
   "md" '(nil :which-key "by date")
   :desc "Goto previous note" "mdb" #'org-roam-dailies-goto-previous-note
   :desc "Goto date"          "mdd" #'org-roam-dailies-goto-date
   :desc "Capture date"       "mdD" #'org-roam-dailies-capture-date
   :desc "Goto next note"     "mdf" #'org-roam-dailies-goto-next-note
   :desc "Goto tomorrow"      "mdm" #'org-roam-dailies-goto-tomorrow
   :desc "Capture tomorrow"   "mdM" #'org-roam-dailies-capture-tomorrow
   :desc "Capture today"      "mdn" #'org-roam-dailies-capture-today
   :desc "Goto today"         "mdt" #'org-roam-dailies-goto-today
   :desc "Capture today"      "mdT" #'org-roam-dailies-capture-today
   :desc "Goto yesterday"     "mdy" #'org-roam-dailies-goto-yesterday
   :desc "Capture yesterday"  "mdY" #'org-roam-dailies-capture-yesterday
   :desc "Find directory"     "md-" #'org-roam-dailies-find-directory
   "md" '(nil :which-key "node properties")
