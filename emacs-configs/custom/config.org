  ...
  (let ((destination (completing-read "Where does this go? "
    ...
    (cond
     ...
     ((string= destination "someday")
      (org-todo "")
      (org-refile nil nil
                  (list "Someday" "~/org/someday.org" nil
                        (with-current-buffer (find-file-noselect "~/org/someday.org")
                          (point-max))))
        ...
        (if is-habit
            ...
            (let* ((frequency (completing-read "How often? "
              ...
              (org-schedule nil (format "<%s %s>"
                                        ...
                                        full-repeater))
              ;; FIX 1: All these forms are now properly inside the let* body,
              ;; so they execute as part of the habit branch
              (org-refile nil nil
                          (list "Habits" "~/org/actions.org" nil
                                (a3madkour/get-refile-marker "~/org/actions.org" "Habits")))
              (message "Sent to habits"))
          ;; ---- REGULAR ONE-OFF ACTION (else branch of if) ----
          (a3madkour/set-task-properties)
          (when (yes-or-no-p "Schedule it? ")
            (org-schedule nil))
          ;; FIX 2: Use get-refile-marker instead of nil for the position,
          ;; so it refiles UNDER the "Actions" heading instead of as a top-level entry
          (org-refile nil nil
                      (list "Actions" "~/org/actions.org" nil
                            (a3madkour/get-refile-marker "~/org/actions.org" "Actions")))
...
(defun a3madkour/reading-queue-active-count ()
  ...
  (with-current-buffer (find-file-noselect "~/org/queue.org")
  ...
  (let* ((name (read-string "Project name: "))
    ...
    (let ((heading (format (concat "* %s"
      ...
      (when (called-interactively-p 'any)
        (switch-to-buffer (find-file-noselect "~/org/projects.org"))
...
(defun a3madkour/create-weekly-summary ()
  ...
  (interactive)
  (let* ((directory "~/org/weekly_summary/")
...
(use-package org
  ...
  (setq org-capture-templates
		  ...
		  ("n" "New interest (parking lot)" entry
		   (file+headline "~/org/someday.org" "Parking Lot")
		   "* %?\n:PROPERTIES:\n:CAPTURED: %U\n:END:\n"
		   :empty-lines 1)))
...
(use-package org-media-note
  ...
  :defer nil
  :config
  (setq org-media-note-screenshot-image-dir "~/org/notes/imgs/")  ;; Folder to save screenshot
...
(general-define-key
 ...
 "fi"
 '((lambda () (interactive) (find-file "~/org/inbox.org")) :which-key "inbox")
 "fp"
 '((lambda () (interactive) (find-file "~/org/projects.org")) :which-key "projects")
 "fa"
 '((lambda () (interactive) (find-file "~/org/actions.org")) :which-key "actions")
 "fq"
 '((lambda () (interactive) (find-file "~/org/queue.org")) :which-key "queue")
 "ff"
