  ...
  (let ((destination (completing-read "Where does this go? "
    ...
    (cond
     ...
     ((string= destination "someday")
      (org-todo "")  ; remove TODO keyword — not a commitment
      (org-refile nil nil
      ...
      (let ((is-habit (yes-or-no-p "Is this a recurring habit? ")))
        (if is-habit
            (let* ((frequency (completing-read "How often? "
              ...
              (org-schedule nil (format "<%s %s>" (format-time-string "%Y-%m-%d %a") full-repeater))
              (org-refile nil nil
  ...
  (let* ((name (read-string "Project name: "))
         ...
         (heading (format (concat "* %s"
                          ...
                          (if repo
                            ...
                            ""))))
    ;; Check WIP limit if activating
    (when (string= status "active")
      (let ((active-count 0))
        (with-current-buffer (find-file-noselect "~/org/projects.org")
          (org-map-entries
           (lambda () (cl-incf active-count))
           "+project+active"))
        (when (>= active-count 3)
          (unless (yes-or-no-p
                   (format "You already have %d active projects. Continue? " active-count))
            (setq heading (replace-regexp-in-string ":active:" ":resting:" heading))
            (message "Set to resting instead")))))
    (with-current-buffer (find-file-noselect "~/org/projects.org")
      (goto-char (point-max))
      (insert heading)
      (save-buffer)
...
(defun a3madkour/create-project-notes ()
  ...
  (let* ((title (org-get-heading t t t t))
         (note-title (read-string "Note title: " (concat title " Notes")))
    ...
    ;; Link from project heading
    (org-end-of-meta-data t)
    (insert (format "Context: [[id:%s][%s]]\n" id note-title))
    (save-buffer)
    ;; Open the note
    (find-file filepath)
    (message "Created and linked: %s" note-title)))
...
;; Toggle a project between active and resting
(defun a3madkour/toggle-project-active ()
  ...
  (let ((tags (org-get-tags)))
    (cond
     ...
     ((member "resting" tags)
      ;; Check WIP limit before activating
      (let ((active-count 0))
        (org-map-entries
         (lambda () (cl-incf active-count))
         "+project+active"
        ...
        (if (>= active-count 3)
            (if (yes-or-no-p
                 (format "You already have %d active projects. Activate anyway? "
                         active-count))
                (progn
                  (org-toggle-tag "resting" 'off)
                  (org-toggle-tag "active" 'on)
                  (message "Project activated (WIP limit exceeded)"))
              (message "Kept resting"))
          (org-toggle-tag "resting" 'off)
          (org-toggle-tag "active" 'on)
          (message "Project activated (%d/3 active)" (1+ active-count)))))
     (t
      (org-toggle-tag "active" 'on)
      (message "Project set to active")))))

(defun a3madkour/add-to-reading-queue ()
  "Add the current org-roam buffer to reading-queue.org."
  (interactive)
  ...
  (let* ((title (org-get-title))
         ...
         (entry (format "* TODO Read %s\n:PROPERTIES:\n:ENERGY: high\n:ROAM_ID: %s\n:END:\n[[id:%s][Open Notes]]\n\n"
                        (or title "Untitled") (or id "") (or id ""))))
    (with-current-buffer (find-file-noselect "~/org/queue.org")
      (goto-char (point-max))
      (insert entry)
      (save-buffer))
    (message "Added \"%s\" to reading queue" title)))

(defun a3madkour/citar-add-to-reading-queue ()
  "Select a citar reference and add it to reading-queue.org."
  (interactive)
...
(defun a3madkour/reading-queue-open-notes ()
  "Open citar notes for the CITEKEY at point in reading-queue.org."
  (interactive)
  (if-let ((key (org-entry-get nil "CITEKEY")))
      (citar-open-notes (list key))
    (if-let ((id (org-entry-get nil "ROAM_ID")))
        (org-id-goto id)
      (user-error "No CITEKEY or ROAM_ID property on this heading"))))
