...
(defun a3madkour/create-project ()
  ...
  (let* ((name (read-string "Project name: "))
         ...
         (heading (format "* %s %s%s\n:PROPERTIES:\n:TRIGGER: next-sibling todo!(NEXT)\n:ORDERED: t\n:DONE_ENOUGH: %s\n%s:END:\n\n** TODO \n"
                          name
                          (format ":%s:%s:project:" area status)
                          ""
                          done-enough
                          (if repo (format ":REPO: %s\n" (abbreviate-file-name repo)) ""))))
    ...
    (with-current-buffer (find-file-noselect "~/org/projects.org")
      ...
      (insert heading)
      (save-buffer)
      ;; Put cursor on the first TODO so you can type the first task
...
;; Quick view of active vs resting projects
(defun a3madkour/show-project-status ()
  "Display active and resting projects in a temporary buffer."
  (interactive)
  (let ((active '())
        (resting '()))
    (with-current-buffer (find-file-noselect "~/org/projects.org")
      (org-map-entries
       (lambda ()
         (let ((title (org-get-heading t t t t)))
           (cond
            ((member "active" (org-get-tags)) (push title active))
            ((member "resting" (org-get-tags)) (push title resting)))))
       "+project"))
    (with-output-to-temp-buffer "*Project Status*"
      (princ (format "Season: %s\n\n" (a3madkour/get-current-season)))
      (princ (format "Active (%d):\n" (length active)))
